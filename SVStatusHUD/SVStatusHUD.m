...
//        CGFloat width = self.hudView.bounds.size.width;
//        CGFloat height = self.hudView.bounds.size.height;
        /*
        self.spin = [[UIActivityIndicatorView alloc]initWithFrame:CGRectMake(0, 0, 160, 120)];
        [self.spin setActivityIndicatorViewStyle:UIActivityIndicatorViewStyleWhiteLarge];
//        self.spin.center = CGPointMake(self.hudView.bounds.size.width/2, self.hudView.bounds.size.height/2);
        [self.hudView addSubview:self.spin];
         */
    }
...
- (UILabel *)stringLabel {
    if (stringLabel == nil) {
        stringLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 40, 160, 20)];
