...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        hInterval = 50;
        vInterval = ( [[UIScreen mainScreen]bounds].size.width - 30 ) / 9;
...
- (void)drawRect:(CGRect)rect
{
    ...
    //纵坐标
    int x = [[UIScreen mainScreen]bounds].size.width - 10 ;
    int y = [[UIScreen mainScreen]bounds].size.height * 3 / 4 ;
    ...
    //横坐标
    int hdConut = [hDesc count];
//    int detX[] = {1,3,5,7,9};
    for (int i=0; i < hdConut; i++) {
        
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(i*vInterval+15, [[UIScreen mainScreen]bounds].size.height * 3 / 4 - 30, 40, 30)];
        [label setBackgroundColor:[UIColor clearColor]];
        [label setTextColor:[UIColor blackColor]];
	...
	//绘图
    int yy = [[UIScreen mainScreen]bounds].size.height * 3 / 4 - 30 ;
    if( [array count] > 0 ){
        
        CGPoint p1 = [[array objectAtIndex:0] CGPointValue];
        int i = 1;
        CGContextMoveToPoint(context, p1.x + 30 , yy-p1.y*hInterval/20);
        for (; i<[array count]  ; i++){
            p1 = [[array objectAtIndex:i] CGPointValue];
            CGPoint goPoint = CGPointMake(p1.x + 30, yy-p1.y*hInterval/20);
